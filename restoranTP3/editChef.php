...
if (isset($_POST["submit"])) {
  ...
  $sql = "UPDATE `chef` SET `name`='$name',`specialization`='$specialization' WHERE id = $id";

  $result = mysqli_query($conn, $sql);

  if ($result) {
    header("Location: chef.php?msg=Data updated successfully");
  } else {
...
<nav class="navbar navbar-light justify-content-center fs-3 mb-3" style="background-color: gray; color:white">
  ...
  <div class="container">
    <?php
    $sql = "SELECT * FROM `chef` WHERE id = $id LIMIT 1";
    $result = mysqli_query($conn, $sql);
